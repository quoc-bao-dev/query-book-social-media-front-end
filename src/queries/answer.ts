...
const getAnswerByQuestion = (questionId: string) => axiosClient.get<HttpResponse<any[]>>(`/answers/${questionId}`).then(respone =>respone.data.data);
...
const postAnswer = (questionId: string, payload:AnswerPayload ) => axiosClient.post('/answers', { questionId ,...payload})
