...
const Page = () => {
  const { data, isLoading } = useFriendsQuery(); // Lấy danh sách bạn bè từ API

  if (isLoading) {
    return <div>Loading...</div>;
  }

  const friends = data?.data?.data || []; // Truy cập danh sách bạn bè từ API response
      ...
      {friends.map((friend) => (
        ...
        >
          ...
          <div className='mt-3 w-full flex flex-col items-center text-center gap-2'>
            ...
            <button
              className='bg-card text-neutral-900 px-5 py-2 rounded-full text-xs font-semibold transition 
