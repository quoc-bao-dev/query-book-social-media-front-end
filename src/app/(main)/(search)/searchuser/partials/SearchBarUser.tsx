...
const SearchBarUser = ({ onSearch }: { onSearch: (query: string) => void }) => {
  const [query, setQuery] = useState('');
  const [searchedQuery, setSearchedQuery] = useState<string | null>(null);

  const handleSearch = () => {
    onSearch(query);
    setSearchedQuery(query); // Lưu lại từ khóa đã tìm
  };
    ...
    <div className='mb-6'>
      ...
      {searchedQuery && (
        <p className='mt-4 text-gray-700 text-lg font-semibold'>
          Result for : <span className='text-green-600'>{searchedQuery}</span>
        </p>
      )}
    </div>
  );
